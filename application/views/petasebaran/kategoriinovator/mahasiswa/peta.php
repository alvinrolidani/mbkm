...
<script>
    ...
    foreach ($getId as $row) {
        $query = $this->db->query("SELECT inovasi.id_inovasi FROM `inovasi` JOIN inovator ON inovasi.id_inovator = inovator.id_inovator WHERE inovator.id_kategori_inovator = 1 AND inovasi.id_kecamatan = '" . $row->id_kecamatan . "'");
    ...
    $getId = $this->db->get('tb_kecamatan')->result();
    foreach ($getId as $row) {
        $query = $this->db->query("SELECT inovator.id_inovator FROM `inovator` WHERE inovator.id_kategori_inovator = 1 AND inovator.id_kecamatan ='" . $row->id_kecamatan . "'");
        $data[$row->nama_kecamatan] = $query->num_rows();
    }
    ...
    <?php foreach ($kecamatan as $kecamatan) : ?>
        ...
        var layer = {
            layer: $.getJSON("<?= base_url('assets/datakecamatan/' . $kecamatan->shp) ?>", function(data) {
                ...
                }).addTo(map).bindPopup('<b style="font-size:15px">Kecamatan ' + Nama + '</b><hr style="color:#f5ce42;margin-top:1px"size="7px">' + '<b>Inovasi : </b>' + HOTSPOT[Nama] + '<br><b>Inovator : </b>' + INOVATOR[Nama] + '<br><a style="margin-left:120px;text-decoration:none; color:black" href="<?= base_url('detailinovator/Mahasiswa/' . $kecamatan->id_kecamatan) ?>">Detail<i class="fa fa-info-circle" style="margin-left:5px" aria-hidden="true"></i></a>')
